...
#TODO add numba-compatible shorthand functions for some computations
...
class MoonPhase:
    ...
    def __repr__(self):
        ...
        return "<%s(%d)>" % (self.__class__.__name__, self.date.timestamp())
...
def phase_string(p):
    ...
    i = bisect.bisect([a[0] for a in _g_phase_strings], p)

    return _g_phase_strings[i][1]
...
def get_fast_defs():
    ...
    @jit
    def phase_fast(jdn_phase_date:int, retvec=False):
        # JDN stands for Julian Day Number
        # Angles here are in degrees
        ...
        moon_dist = (moon_smaxis * (1 - moon_eccentricity**2))\
            / (1 + moon_eccentricity * cos(torad(MmP + mEc)))
        ...
        res = (
            ...
            moon_phase,
            synodic_month * fixangle(moon_age) / 360.0,
            moon_dist,
            moon_angular_diameter,
            sun_dist,
            sun_angular_diameter
        )
        # res['all'] = [v for k,v in res.items()]
        
        return res

    # @jit(objectmode=True)
    def phase_vec(dates:numba.int64[:]):
        # phases = np.ndarray((len(dates), 7))
        phases = np.zeros((len(dates), 7))
        for i in range(len(dates)):
            phases[i, :] = phase_fast(dates[i])
        return phases
    return (phase_fast, phase_vec)
...
def phase(phase_date=DateTime.now()):
    ...
    return res
# phase()
jit_defs = get_fast_defs()
phase.fast = jit_defs[0]
phase.afast = jit_defs[1]
...
##
#
def is_full_moon(d=DateTime.now()):
    m = MoonPhase(d)
    return (m.illuminated >= 0.95)
