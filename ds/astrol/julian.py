...
def __to_format(jd: float, fmt: str):
    """
...
def __from_format(jd: float, fmt: str):
    ...
    if fmt.lower() == 'jd':
        ...
        # the day
        return floor(jd + 0.5), jd + 0.5 - floor(jd + 0.5)
    elif fmt.lower() == 'mjd':
...
def to_jd(dt: datetime, fmt: str = 'jd'):
    """
    ...
    jdn = dt.day + floor((153*m + 2)/5) + 365*y + floor(y/4) - \
...
def from_jd(jd: float, fmt: str = 'jd'):
    """
