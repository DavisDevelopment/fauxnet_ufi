...
class FileDict(object):
    ...
    def __init__(self, solution=Solutions.Sqlite3, **options):
        ...
        self.__conn.execute(
            'create table if not exists %s (hash integer, key blob, value blob);' % self.__tablename)
        self.__conn.execute('create index if not exists %s_index ON %s(hash);' % (
    ...
    def __getitem__(self, key):
        ...
        c = self.__conn.execute(
            'select value from %s where hash=? and key=?;' % tbl, (key_hash, key_pickle))
        res = c.fetchone()
        if res is None:
            raise KeyError(key)
    ...
    def __setitem(self, key, value):
        ...
        res = self.__conn.execute(
            'update %s set value=? where hash=? and key=?;' % tbl, (value_pickle, key_hash, key_pickle))
        if res.rowcount <= 0:
            res = self.__conn.execute(
                'insert into %s values (?, ?, ?);' % tbl, (key_hash, key_pickle, value_pickle))
    ...
    def __delitem__(self, key):
        ...
        res = self.__conn.execute(
            'delete from %s where hash=? and key=?;' % tbl, (key_hash, key_pickle))
        if res.rowcount <= 0:
            raise KeyError(key)
    ...
    def has_key(self, key):
        ...
        key_pickle = self.__pack_key(key)
        c = self.__conn.execute(
            'select count(*) from %(tbl)s where hash=%(key_hash)d and key=%(key_pickle)r;' % locals())
        res = c.fetchone()
        assert res
        assert 0 <= res[0] <= 1
