...
# @script
def _build_classifier_tree(vectors:Tensor, labels:Tensor, func, depth:int):
    """
    ...
    # print(f'_build_tree(..., depth={depth}, column_count={column_count}, n_labels={len(labels)}, n_vectors={len(vectors)})')
    ...
    if best_gain > 0:
        true_branch =  _build_classifier_tree(best_sets[0][0], best_sets[0][1], func, depth - 1)
        false_branch = _build_classifier_tree(best_sets[1][0], best_sets[1][1], func, depth - 1)
...
class TorchDecisionTreeClassifier(torch.nn.Module):
    ...
    def fit(self, vectors:Tensor, labels:Tensor, criterion=None):
        """
        -
    ...
    def _build_tree(self, vectors:Tensor, labels:Tensor, func, depth:int):
        """
        Private recursive function used to build the tree.
        """
        if len(vectors) == 0:
    ...
    def _classify(self, vector, node):
        ...
        if node.results is not None:
            ...
            cond:Tensor = split_function(vector, node.col, node.value)
            # print(cond)
            if cond.any():
                branch = node.tb
            else:
...
class TorchDecisionTreeRegressor(torch.nn.Module):
    ...
    """
    _root_node:Optional[DecisionNode]
    max_depth:int
    ...
    def _build_tree(self, vectors, values, func, depth:int):
        """
        Private recursive function used to build the tree.
        """
        if len(vectors) == 0:
    ...
    def _regress(self, vector, node:DecisionNode):
        """
        ...
        if node.results is not None:
            ...
            return self._regress(vector, branch)#type: ignore
